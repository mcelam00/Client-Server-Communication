...
int main(int argc, char** argv) {
    ...
    while (i < MAXITERATIONS) {
        int nbytes = recvfrom(s, DATEREQUEST, REQLENGTH, 0, (struct sockaddr *) &client, &addr_length); //addr_lenght lo paso como puntero porque puede cambiar en funcion de lo que llegue en el argumento anterior, si fuera fijo sería invarible y calculado solo en base a una primera copia donde estaría vacia client

        printf("Solicitud recibida con un tamaño de %u bytes, procedentes de %s@ puerto %u:\n",
                nbytes, inet_ntoa(client.sin_addr), client.sin_port); //la complementaria a inet_addr("");

        printf("'%s'\n", DATEREQUEST);

        char *response = "Respuesta de prueba"; //es lo que vamos a responder al cliente
